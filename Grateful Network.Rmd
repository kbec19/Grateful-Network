---
title: "Grateful Dead Songwriting Network"
description: |
  Ongoing Project Exploring the Grateful Dead Original Song Co-Writing Network
categories:
  - networks
  - grateful network
author:
  - name: Kristina Becvar
slug: grateful-network
date: 04/20/2022
output:
...
knitr::opts_knit$set(root.dir = "~/GitHub/Grateful Network/Grateful-Network")
```
...
For my project, I am using the Grateful Dead song writers data set that I am using in [this series of posts](https://www.kristinabecvar.com/blog.html#category:grateful_network){target="_blank"} to examine the network features of the co-writers of original Grateful Dead songs.
...
```{r code_folding=TRUE}
gd_nodes <- read.csv('./_data/gd_nodes.csv', row.names = 1, header = TRUE, check.names = FALSE)
gd_affiliation <- read.csv('./_data/gd_affiliation_matrix.csv', row.names = 1, header = TRUE, check.names = FALSE)
...
Coercing directly from the original affiliation matrix kept giving the error, *"Error: `loops` is `FALSE`, but `x` contains loops."* I tried using the "intergraph" package, but it cannot coerce bipartite igraph networks. After the bipartite projection, I was able to create the statnet object as well.
...
centrality_gd_igraph <- read.csv("./_data/centrality_df.csv")
...
Next, looking at the network basics
